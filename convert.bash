              ...
              do
                  case $location in
                      "Original directory")
                          # Create a backup folder
                          mkdir -p "$dir/backup"
                          ffmpeg -i "$file" -c:v libx264 -c:a aac -strict experimental -b:a 128k -b:v 1200k -r 30 -progress pipe:1 "$dir/backup/${filename%.*}.mp4"
                          mv "$file" "$dir/backup/"
                      ...
                      "Custom location")
                          ...
                          ffmpeg -i "$file" -c:v libx264 -c:a aac -strict experimental -b:a 128k -b:v 1200k -r 30 -progress pipe:1 "$custom_location/${filename%.*}.mp4"
